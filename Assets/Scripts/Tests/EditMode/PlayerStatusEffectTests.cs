...
namespace GangHoBiGeup.Tests
{
    ...
    public class PlayerStatusEffectTests
    {
        ...
        public void Player에게_상태이상을_적용할_수_있다()
        {
            ...
            var statusEffect = new StatusEffect(StatusEffectType.Strength, 2, 3);
        ...
        public void Player에게_힘_버프를_적용하면_공격력이_증가한다()
        {
            // Arrange
            var player = new GameObject().AddComponent<Player>();
            var strengthBuff = new StatusEffect(StatusEffectType.Strength, 3, 2);
        ...
        public void Player에게_약화_디버프를_적용하면_공격력이_감소한다()
        {
            // Arrange
            var player = new GameObject().AddComponent<Player>();
            var weakDebuff = new StatusEffect(StatusEffectType.Weak, 2, 2);
        ...
        public void Player에게_취약_디버프를_적용하면_받는_피해가_증가한다()
        {
            ...
            player.Block = 0;
            var vulnerableDebuff = new StatusEffect(StatusEffectType.Vulnerable, 1, 2);
        ...
        public void 턴_종료_시_일시적_상태이상의_지속_시간이_감소한다()
        {
            // Arrange
            var player = new GameObject().AddComponent<Player>();
            var strengthBuff = new StatusEffect(StatusEffectType.Strength, 2, 3); // 3턴 지속
        ...
        public void 같은_상태이상을_중첩_적용하면_수치가_증가한다()
        {
            // Arrange
            var player = new GameObject().AddComponent<Player>();
            var strength1 = new StatusEffect(StatusEffectType.Strength, 2, 2);
            var strength2 = new StatusEffect(StatusEffectType.Strength, 3, 2);
