...
**🎉 전체 완료: 14/14 Phases (100%)**

| Phase | 상태 | 테스트 수 | 비고 |
|-------|------|----------|------|
| Phase 1: 핵심 데이터 모델 | ✅ 완료 | 19/19 | CardData, EnemyData, RelicData, FactionData |
| Phase 2: 플레이어 시스템 | ✅ 완료 | 28/28 | 상태 관리, 덱 관리, 경지 시스템, 상태이상 |
| Phase 3: 적(Enemy) 시스템 | ✅ 완료 | 14/14 | 상태 관리, 행동 패턴, 상태이상 |
| Phase 4: 전투 시스템 | ✅ 완료 | 21/21 | 초기화, 턴 관리, 카드 사용, 전투 종료 |
| Phase 5: 연계 초식 시스템 | ✅ 완료 | 6/6 | 콤보 시스템 |
| Phase 6: 게임 관리 시스템 | ✅ 완료 | 10/10 | 게임 상태, 세이브/로드 |
| Phase 7: 맵 시스템 | ✅ 완료 | 15/15 | 맵 생성, 노드 상호작용 |
| Phase 8: 유물 시스템 | ✅ 완료 | 5/5 | 유물 획득, 트리거 효과 |
| Phase 9: 이벤트 시스템 | ✅ 완료 | 4/4 | 기연 이벤트 |
| Phase 10: UI 시스템 | ✅ 완료 | 8/8 | 전투 UI, 맵 UI |
| Phase 11: 메타 시스템 | ✅ 완료 | 5/5 | 깨달음 포인트, 영구 강화 |
| Phase 12: 심마 전투 시스템 | ✅ 완료 | 7/7 | 폐관수련, 경지 돌파 |
| Phase 13: 문파별 특수 메카닉 | ✅ 완료 | 9/9 | 화산파, 천마신교, 사천당문, 하북팽가 |
| Phase 14: 최종 통합 테스트 | ✅ 완료 | 7/7 | 전체 게임 플로우 |
...
### 4.3 카드 사용
- [x] Test: Player가 카드를 사용할 수 있다
- [x] Test: 카드 사용 시 내공이 소모된다
- [x] Test: 내공이 부족하면 카드를 사용할 수 없다
- [x] Test: 카드 효과가 올바르게 적용된다
- [x] Test: 타겟이 필요한 카드는 타겟을 지정해야 사용할 수 있다
- [x] Test: '절초' 카드는 사용 후 소멸된다

### 4.4 전투 종료
- [x] Test: 모든 적이 사망하면 전투에서 승리한다
- [x] Test: Player가 사망하면 전투에서 패배한다
- [x] Test: 전투 승리 시 수련치를 획득한다
- [x] Test: 전투 승리 시 보상(카드, 골드)를 받는다

## Phase 5: 연계 초식 시스템 ✅

### 5.1 연계 조건
- [x] Test: 연계 초식의 조건을 정의할 수 있다
- [x] Test: 카드 사용 순서를 추적할 수 있다
- [x] Test: 특정 순서로 카드를 사용하면 연계가 발동한다
- [x] Test: 연계 발동 시 추가 효과가 적용된다
...
### 7.1 맵 생성
- [x] Test: MapManager를 생성할 수 있다
- [x] Test: 절차적으로 맵 노드를 생성할 수 있다
- [x] Test: 노드 간 연결 경로를 생성할 수 있다
- [x] Test: 각 층마다 적절한 비율로 노드 타입이 배치된다

### 7.2 노드 상호작용
- [x] Test: Player가 노드를 선택할 수 있다
- [x] Test: 노드 선택 시 해당 노드 타입의 이벤트가 발생한다
- [x] Test: 전투 노드 선택 시 전투가 시작된다
- [x] Test: 휴식처 노드에서 체력 회복/카드 강화/경지 돌파를 선택할 수 있다
- [x] Test: 상점 노드에서 카드를 구매할 수 있다
- [x] Test: 기연 노드에서 이벤트가 발생한다

## Phase 8: 유물 시스템 ✅

### 8.1 유물 획득과 효과
- [x] Test: Player가 유물을 획득할 수 있다
- [x] Test: 전투 시작 시 발동하는 유물이 작동한다
- [x] Test: 턴 시작 시 발동하는 유물이 작동한다
- [x] Test: 카드 사용 시 발동하는 유물이 작동한다
- [x] Test: 여러 유물의 효과가 동시에 적용된다

## Phase 9: 이벤트 시스템 ✅

### 9.1 기연 이벤트
- [x] Test: EventData로 이벤트를 정의할 수 있다
- [x] Test: 이벤트 선택지를 표시할 수 있다
- [x] Test: 선택지 선택 시 결과가 적용된다
- [x] Test: 확률적 결과가 있는 이벤트가 작동한다
...
### 11.1 깨달음 시스템
- [x] Test: 게임 종료 시 깨달음 포인트를 획득한다
- [x] Test: 깨달음 포인트로 영구 강화를 구매할 수 있다
- [x] Test: 영구 강화가 새 게임에 적용된다

## Phase 12: 심마 전투 시스템 ✅

### 12.1 폐관수련과 심마
- [x] Test: 휴식처에서 폐관수련을 선택할 수 있다
- [x] Test: 폐관수련 선택 시 심마와의 전투가 시작된다
- [x] Test: 심마 전투에서 승리하면 경지가 상승한다
- [x] Test: 심마 전투에서 패배하면 주화입마 상태가 된다

## Phase 13: 문파별 특수 메카닉 ✅

### 13.1 화산파 연계 시스템
- [x] Test: 화산파 전용 연계 초식이 작동한다
- [x] Test: 연계 콤보 카운터가 작동한다

### 13.2 천마신교 체력 소모 시스템
- [x] Test: 체력을 대가로 하는 카드가 작동한다
- [x] Test: 천마 상태가 올바르게 적용된다

### 13.3 사천당문 중독 시스템
- [x] Test: 중독 중첩이 올바르게 작동한다
- [x] Test: 중독 시너지 카드가 작동한다

### 13.4 하북팽가 무방비/힘 시스템
- [x] Test: 무방비 상태가 올바르게 적용된다
- [x] Test: 무방비와 힘의 시너지가 작동한다

## Phase 14: 최종 통합 테스트 ✅

### 14.1 전체 게임 플로우
- [x] Test: 메인 메뉴에서 게임을 시작할 수 있다
- [x] Test: 문파를 선택하고 게임을 시작할 수 있다
- [x] Test: 맵을 탐험하며 전투를 진행할 수 있다
- [x] Test: 3층 보스를 격파하면 게임에서 승리한다
- [x] Test: 사망 시 게임 오버가 된다
- [x] Test: 게임 종료 후 메타 진행이 저장된다
