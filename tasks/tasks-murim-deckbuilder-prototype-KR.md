...
- [x] 6.0 전투 UI 및 시각적 피드백 생성
  - [x] 6.1 CardPrefab 생성: 카드 이미지 배경, 이름 텍스트, 비용 텍스트, 설명 텍스트, 타입 아이콘
  - [x] 6.2 카드 데이터를 시각적으로 표시하는 `CardUI.cs` 컴포넌트 생성
  - [x] 6.3 카드 호버 효과 구현 (확대, 툴팁 표시)
  - [x] 6.4 카드를 사용하기 위한 드래그 앤 드롭 구현
  - [x] 6.5 패 레이아웃 매니저 생성 (카드 가로 배치)
  - [x] 6.6 체력 바와 내공 카운터를 표시하는 `PlayerStatsUI.cs` 생성
  - [x] 6.7 적 의도 아이콘과 값을 표시하는 `EnemyIntentUI.cs` 생성
  - [x] 6.8 적 체력 바 UI 생성
  - [x] 6.9 onClick 리스너를 가진 "턴 종료" 버튼 생성
  - [x] 6.10 클릭하여 볼 수 있는 더미 카운터 추가 (뽑기 더미, 버리기 더미, 소진 더미)
  - [x] 6.11 피해 숫자 팝업 애니메이션 구현 (간단한 텍스트 페이드 아웃)
  - [x] 6.12 카드 사용에 대한 시각적 피드백 추가 (빛나는 효과, 이동 애니메이션)
  - [x] 6.13 플레이어/적에 방어 방패 아이콘 표시 추가
  - [x] 6.14 "메뉴로 돌아가기" 버튼이 있는 승리/패배 화면 생성
  - [x] 6.15 모든 UI 상호작용 및 반응성 테스트

- [x] 7.0 초기 카드 세트 생성 (20장)
  - [x] 7.1 PRD 사양에 따라 7장의 공격 카드 디자인:
    - [x] 7.1.1 일검 (Strike) - 1 내공, 6 피해
    - [x] 7.1.2 쌍검난무 (Double Strike) - 1 내공, 3 피해 2회
    - [x] 7.1.3 참마검 (Heavy Slash) - 2 내공, 12 피해
    - [x] 7.1.4 검기참 (Sword Wave) - 1 내공, 7 피해
    - [x] 7.1.5 연환삼검 (Triple Slash) - 2 내공, 4 피해 3회
    - [x] 7.1.6 용검섬 (Dragon Sword Flash) - 1 내공, 8 피해
    - [x] 7.1.7 천검귀일 (Thousand Swords Return) - 3 내공, 20 피해
  - [x] 7.2 7장의 방어 카드 디자인:
    - [x] 7.2.1 철포삼 (Iron Guard) - 1 내공, 방어도 5 획득
    - [x] 7.2.2 신법 (Swift Dodge) - 1 내공, 방어도 6 획득
    - [x] 7.2.3 무영신법 (Shadow Step) - 2 내공, 방어도 12 획득
    - [x] 7.2.4 방어자세 (Defensive Stance) - 1 내공, 방어도 7 획득
    - [x] 7.2.5 금종죄 (Golden Bell) - 2 내공, 방어도 10 획득
    - [x] 7.2.6 회광반조 (Reflect) - 1 내공, 방어도 4 획득
    - [x] 7.2.7 절대방어 (Absolute Defense) - 3 내공, 방어도 18 획득
  - [x] 7.3 6장의 스킬 카드 디자인:
    - [x] 7.3.1 내공운기 (Qi Circulation) - 1 내공, 카드 2장 드로우
    - [x] 7.3.2 심검합일 (Sword Heart Unity) - 0 내공, 다음 공격 +3 피해
    - [x] 7.3.3 명경지수 (Clear Mind) - 1 내공, 방어도 3 획득, 카드 1장 드로우
    - [x] 7.3.4 검기방출 (Sword Aura Release) - 2 내공, 모든 적에게 10 피해, 소진
    - [x] 7.3.5 기혈순환 (Energy Flow) - 1 내공, 체력 5 회복
    - [x] 7.3.6 천지무극 (Heaven Earth Infinite) - 2 내공, 카드 3장 드로우, 방어도 5 획득
  - [x] 7.4 적절한 형식으로 CardDatabase.json에 모든 20장의 카드 추가
  - [x] 7.5 JSON 문법 및 카드 밸런스 검증
  - [x] 7.6 게임 내 모든 카드 로딩 테스트

- [x] 8.0 적 타입 생성 및 밸런스 테스트
  - [x] 8.1 적 타입 1 생성: 산적 (Bandit)
    - [x] 8.1.1 스탯: 40 HP
    - [x] 8.1.2 행동: 공격 (6 피해, 60% 가중치), 방어 (8 방어도, 40% 가중치)
  - [x] 8.2 적 타입 2 생성: 마도수련자 (Dark Cultivator)
    - [x] 8.2.1 스탯: 30 HP
    - [x] 8.2.2 행동: 공격 (8 피해, 50%), 버프 (+2 힘, 30%), 공격 (4 피해 2회, 20%)
  - [x] 8.3 적 타입 3 생성: 철갑위병 (Armored Guard)
    - [x] 8.3.1 스탯: 50 HP
    - [x] 8.3.2 행동: 공격 (5 피해, 40%), 방어 (12 방어도, 40%), 강 공격 (15 피해, 20%)
  - [x] 8.4 EnemyDatabase.json에 모든 적 데이터 추가
  - [x] 8.5 각 적 타입을 개별적으로 테스트하는 씬 생성
  - [x] 8.6 밸런스 테스트: 플레이어가 시작 덱으로 각 적을 물리칠 수 있는가?
  - [x] 8.7 플레이테스트 피드백을 기반으로 카드/적 스탯 조정
  - [x] 8.8 밸런스 변경 사항 및 이유 문서화

- [x] 9.0 통합 테스트 및 폴리싱
  - [x] 9.1 포괄적인 플레이테스트 시나리오 생성 (플레이어 vs 각 적 타입)
  - [x] 9.2 엣지 케이스 테스트:
    - [x] 9.2.1 빈 덱에서 드로우 (버리기 더미 재셔플해야 함)
    - [x] 9.2.2 패 제한 초과 (10장 이상 드로우 방지해야 함)
    - [x] 9.2.3 내공 부족으로 카드 사용 (방지되어야 함)
    - [x] 9.2.4 전투 중 적 격파 (승리로 전환되어야 함)
    - [x] 9.2.5 플레이어 격파 (패배 화면으로 전환되어야 함)
  - [x] 9.3 발견된 버그 수정
  - [x] 9.4 기본 효과음 추가 (카드 사용, 피해, 승리, 패배) - 플레이스홀더 오디오 허용
  - [x] 9.5 성능 최적화 (60 FPS 목표)
  - [x] 9.6 테스트를 위한 디버그 콘솔/치트 명령 추가 (무적 모드, 즉시 승리, 카드 생성)
  - [ ] 9.7 Windows용 빌드 생성 (실행 파일)
  - [ ] 9.8 2-3명의 외부 테스터와 최종 플레이테스트 진행
  - [x] 9.9 알려진 이슈 및 향후 개선 사항 문서화
